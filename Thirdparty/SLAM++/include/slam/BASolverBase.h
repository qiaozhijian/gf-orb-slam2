...
// enable analytical jacobian (so3); currently only approximated
#define ENABLE_ANALYTICAL_JACOBIAN

// remove distortion computation in analytical Jacobian module
...
public:
    ...
    typedef Eigen::Matrix<double, 3, 3> Matrix3d; /**< @brief 7x7 matrix type */
...
public:
    ...
    static void Project_P2C_Quat(const Eigen::MatrixBase<Derived0> &r_t_vertex1,
    ...
    {
        DimensionCheck<Vector7d>(r_t_vertex1);
        DimensionCheck<Vector5d>(r_t_intrinsics);
        DimensionCheck<Eigen::Vector3d>(r_t_vertex2);
        DimensionCheck<Eigen::Vector2d>(r_t_dest);
        ...
        double cx = r_t_intrinsics(2);
        double cy = r_t_intrinsics(3);
#ifndef DISABLE_DISTORTION
        double k = r_t_intrinsics(4) / (.5 * (fx + fy))/*sqrt(fx * fy)*/; // rescale intrinsics
        // since post-ICRA2015 release the distortion is scaled by focal length in the internal representation
#endif
        ...
        Rt.template block<3, 3>(0, 0) = C3DJacobians::t_Quat_to_RotMatrix(r_t_vertex1.template tail<4>());
...
#ifdef DISABLE_DISTORTION
        // skip distortion part
#else
    ...
    /**
     *	@brief projects a 3D point to 2D camera image and calculates the jacobians analatically
     *
     *	@tparam Derived0 is Eigen derived matrix type for the first matrix argument
     *	@tparam Derived1 is Eigen derived matrix type for the second matrix argument
     *	@tparam Derived2 is Eigen derived matrix type for the third matrix argument
                                 ...
                                 Eigen::MatrixBase<Derived4> &r_t_pose3_pose1, Eigen::MatrixBase<Derived5> &r_t_pose3_pose2)
    {
        ...
        DimensionCheck<Eigen::Vector3d>(r_t_vertex2);
        DimensionCheck<Eigen::Vector2d>(r_t_dest);
        DimensionCheck<Eigen::Matrix<double, 2, 7> >(r_t_pose3_pose1);
        ...
        //construct [R | t]
        Eigen::Matrix<double, 3, 4> RaT;
        RaT.template block<3, 3>(0, 0) = C3DJacobians::t_Quat_to_RotMatrix(r_t_vertex1.template tail<4>());
        RaT.col(3) = r_t_vertex1.template head<3>();
        ...
        //project world to camera
        Eigen::Vector3d x = RaT * X;

#ifdef DEBUG_VERBOSE
        fprintf(stderr, "---------------------\n");
        fprintf(stderr, "camera at: ");
        for(int a = 0; a < 3; a++) {
            fprintf(stderr, "%f ", r_t_vertex1(a));
        }
        fprintf(stderr, "\n");
        fprintf(stderr, "lmk at: ");
        for(int a = 0; a < 3; a++) {
            fprintf(stderr, "%f ", r_t_vertex2(a));
        }
        fprintf(stderr, "\n");
        fprintf(stderr, "in camera coord: ");
        for(int a = 0; a < 3; a++) {
...
#ifdef DEBUG_VERBOSE
        fprintf(stderr, "in image coord: ");
        for(int a = 0; a < 3; a++) {
            fprintf(stderr, "%f ", uv(a));
        }
        fprintf(stderr, "\n");
#endif
        ...
        //lets do analytical Jacobian
        double tx = r_t_vertex1(0); double ty = r_t_vertex1(1); double tz = r_t_vertex1(2);
        double px = r_t_vertex2(0); double py = r_t_vertex2(1); double pz = r_t_vertex2(2);
        double R1 = tx + RaT(0, 0)*px + RaT(0, 1)*py + RaT(0, 2)*pz;
        double R2 = ty + RaT(1, 0)*px + RaT(1, 1)*py + RaT(1, 2)*pz;
        double R3 = tz + RaT(2, 0)*px + RaT(2, 1)*py + RaT(2, 2)*pz;
        double R3_2 = pow(R3, 2);
        //        double R3_r1 = tz + RaT(2, 1)*py + RaT(2, 2)*pz;
        //        double R3_r2 = tz + RaT(2, 0)*px + RaT(2, 2)*pz;
        double fac_x = fx / R3_2, fac_y = fy / R3_2;

        // camera pose part
        r_t_pose3_pose1(0, 0) = R3 * fac_x;
        r_t_pose3_pose1(0, 1) = 0.0f;
        r_t_pose3_pose1(0, 2) = -R1 * fac_x;

        r_t_pose3_pose1(1, 0) = 0.0f;
        r_t_pose3_pose1(1, 1) = R3 * fac_y;
        r_t_pose3_pose1(1, 2) = -R2 * fac_y;

        // the following code seems problematic; redo the math to derive 1st jacobian!!!
        Eigen::Matrix<double, 2, 3> U_p;
        double z_2 = std::pow(x(2), 2.0);
        if ( fabs(uv(2)) < 1e-6 ) {
            U_p.Zero();
        } else {
            U_p << fx/x(2), 0.0, -x(0)*fx/z_2,
                    0.0, fy/x(2), -x(1)*fy/z_2;
        }

        Matrix3d R_t = RaT.template block<3, 3>(0,0);
        R_t.transpose();

        Eigen::Matrix<double, 3, 4> dRq_times_a_by_dq;
        quaternionJacobian(r_t_vertex1.template tail<4>(), r_t_vertex2-r_t_vertex1.template head<3>(), dRq_times_a_by_dq);
        r_t_pose3_pose1.template block<2, 4>(0, 3) = - U_p * dRq_times_a_by_dq;

        // TODO
        // redo the jacobian of camera quaternion
        // H matrix subblock (cols 4~7): H47
        //        Eigen::Vector4d v1;
        //        v1 << 1.0, -1.0, -1.0, -1.0;
        //        Eigen::Matrix4d dqbar_by_dq = v1.asDiagonal();
        //        Eigen::Vector3d t = r_t_vertex2 - r_t_vertex1.template head<3>();
        //        Eigen::Matrix<double, 3, 4> dRq_times_a_by_dq;
        //        quaternionJacobian(r_t_vertex1.template tail<4>(), r_t_vertex2, dRq_times_a_by_dq);
        //        //        r_t_pose3_pose1.template block<2, 4>(0, 3) = dhu_dhrl * (dRq_times_a_by_dq * dqbar_by_dq);
        //        Eigen::Matrix<double, 3, 4> dProj_by_dq;
        //        dProj_by_dq = A * dRq_times_a_by_dq / R3;
        //        r_t_pose3_pose1.template block<1, 4>(0, 3) = dProj_by_dq.template block<1, 4>(0, 0) - dProj_by_dq.template block<1, 4>(2, 0);
        //        r_t_pose3_pose1.template block<1, 4>(1, 3) = dProj_by_dq.template block<1, 4>(1, 0) - dProj_by_dq.template block<1, 4>(2, 0);
        //
        //#ifdef DEBUG_VERBOSE
        //        fprintf(stderr, "dProj_by_dq\n");
        //        for (size_t i=0; i<3; ++i) {
        //            for (size_t j=0; j<4; ++j) {
        //                fprintf(stderr, "%f ", dProj_by_dq(i,j));
        //            }
        //            fprintf(stderr, "\n");
        //        }
        //        fprintf(stderr, "\n");
        //        fprintf(stderr, "r_t_pose3_pose1\n");
        //        for (size_t i=0; i<2; ++i) {
        //            for (size_t j=0; j<4; ++j) {
        //                fprintf(stderr, "%f ", r_t_pose3_pose1(i,j));
        //            }
        //            fprintf(stderr, "\n");
        //        }
        //        fprintf(stderr, "\n");
        ...
        // landmark part
        //        r_t_pose3_pose2(0, 0) = (RaT(0, 0) * R3_r1 - RaT(2, 0) * (tx + RaT(0, 1)*py + RaT(0, 2)*pz)) * fac_x;
        //        r_t_pose3_pose2(0, 1) = (RaT(0, 1) * R3_r2 - RaT(2, 1) * (tx + RaT(0, 0)*px + RaT(0, 2)*pz)) * fac_x;
        //        r_t_pose3_pose2(0, 2) = -((RaT(2, 2)*tx - RaT(0, 2)*tz) + px * (RaT(0, 0)*RaT(2, 2) - RaT(0, 2)*RaT(2, 0)) +
        //                                  py * (RaT(0, 1)*RaT(2, 2) - RaT(0, 2)*RaT(2, 1))) * fac_x;

        //        r_t_pose3_pose2(1, 0) = (RaT(1, 0) * R3_r1 - RaT(2, 0) * (ty + RaT(1, 1)*py + RaT(1, 2)*pz)) * fac_y;
        //        r_t_pose3_pose2(1, 1) = (RaT(1, 1) * R3_r2 - RaT(2, 1) * (ty + RaT(1, 0)*px + RaT(1, 2)*pz)) * fac_y;
        //        r_t_pose3_pose2(1, 2) = -((RaT(2, 2)*ty - RaT(1, 2)*tz) + px * (RaT(1, 0)*RaT(2, 2) - RaT(1, 2)*RaT(2, 0)) +
        //                                  py * (RaT(1, 1)*RaT(2, 2) - RaT(1, 2)*RaT(2, 1))) * fac_y;

        //        Eigen::Matrix<double, 2, 3> H_l = U_p *  R_t;
        r_t_pose3_pose2 = U_p *  R_t;

#ifdef DEBUG_VERBOSE
        // DEBUG
        // verify the correctness of analatical Jacobians
        ...
        Eigen::Matrix<double, 2, 7> num_H1;
        Eigen::Matrix<double, 2, 3> num_H2;
        // can actually work inplace

        Eigen::Vector2d d1;
        Vector7d p_delta;
        Eigen::Vector3d p_delta2;
        Eigen::Quaterniond quat_base(r_t_vertex1(3), r_t_vertex1(4), r_t_vertex1(5), r_t_vertex1(6));
        //        fprintf(stderr, "qw: %f %f\n", quat_base.w(), r_t_vertex1(3));
        //        fprintf(stderr, "qx: %f %f\n", quat_base.x(), r_t_vertex1(4));
        //        fprintf(stderr, "qy: %f %f\n", quat_base.y(), r_t_vertex1(5));
        //        fprintf(stderr, "qz: %f %f\n", quat_base.z(), r_t_vertex1(6));

        for(int j = 0; j < 7; ++ j) {
            // add disturbance to translation or unit quaternion
            p_delta = r_t_vertex1;
            if (j < 3) {
                p_delta(j) += delta;
            }
            else if (j == 3) {
                Eigen::Quaterniond quat_dist(1, 0, 0, 0);
                quat_dist = C3DJacobians::t_Quat_Production(quat_dist, quat_base);
                //                quat_dist.normalize();
                p_delta(3) = quat_dist.w();
                p_delta(4) = quat_dist.x();
                p_delta(5) = quat_dist.y();
                p_delta(6) = quat_dist.z();
            }
            else if (j == 4) {
                Eigen::Quaterniond quat_dist(sqrt(1-delta*delta), delta, 0, 0);
                quat_dist = C3DJacobians::t_Quat_Production(quat_dist, quat_base);
                //                quat_dist.normalize();
                p_delta(3) = quat_dist.w();
                p_delta(4) = quat_dist.x();
                ...
                Eigen::Quaterniond quat_dist(sqrt(1-delta*delta), 0, delta, 0);
                quat_dist = C3DJacobians::t_Quat_Production(quat_dist, quat_base);
                //                quat_dist.normalize();
                p_delta(3) = quat_dist.w();
                p_delta(4) = quat_dist.x();
                ...
                Eigen::Quaterniond quat_dist(sqrt(1-delta*delta), 0, 0, delta);
                quat_dist = C3DJacobians::t_Quat_Production(quat_dist, quat_base);
                //                quat_dist.normalize();
                p_delta(3) = quat_dist.w();
                p_delta(4) = quat_dist.x();
            ...
            //                        p_delta(3), p_delta(4), p_delta(5), p_delta(6));
            Project_P2C_Quat(p_delta, r_t_intrinsics, r_t_vertex2, d1);
            //            fprintf(stderr, "%f %f vs. %f %f, with disturbance %f\n",
            //                    d1(0), d1(1), r_t_dest(0), r_t_dest(1), p_delta(j) - r_t_vertex1(j));
        ...
        for(int j = 0; j < 3; ++ j) {
            ...
            Project_P2C_Quat(r_t_vertex1, r_t_intrinsics, p_delta2, d1);
            _ASSERTE( fabs( d1(0) - uv(0) ) < 1e-7 && fabs( d1(1) - uv(1) ) < 1e-7 );
            //            fprintf(stderr, "%f %f vs. %f %f\n", d1(0), d1(1), uv(0), uv(1));

            num_H2.col(j) = (uv.template head<2>() - r_t_dest) * scalar;
        }

        //        fprintf(stderr, "-------------------\n\n");
        //        fprintf(stderr, "r_t_pose3_pose1 v1\n");
        //        for(int a = 0; a < 2; a++) {
        //            for(int b = 0; b < 7; b++) {
        //                fprintf(stderr, "%f ", old_jacob(a,b));
        //            }
        //            fprintf(stderr, "\n");
        //        }
        fprintf(stderr, "\n");
        fprintf(stderr, "H1 analytical\n");
        for(int a = 0; a < 2; a++) {
            for(int b = 0; b < 7; b++) {
                fprintf(stderr, "%f ", r_t_pose3_pose1(a,b));
            }
            fprintf(stderr, "\n");
        }
        fprintf(stderr, "\n");
        fprintf(stderr, "H1 numerical\n");
        for(int a = 0; a < 2; a++) {
            for(int b = 0; b < 7; b++) {
                fprintf(stderr, "%f ", num_H1(a,b));
            }
            fprintf(stderr, "\n");
        }
        fprintf(stderr, "\n");
        fprintf(stderr, "H2 analytical\n");
        for(int a = 0; a < 2; a++) {
            for(int b = 0; b < 3; b++) {
                fprintf(stderr, "%f ", r_t_pose3_pose2(a,b));
            }
            fprintf(stderr, "\n");
        }
        //        fprintf(stderr, "\n");
        //        fprintf(stderr, "H2 analytical v2\n");
        //        for(int a = 0; a < 2; a++) {
        //            for(int b = 0; b < 3; b++) {
        //                fprintf(stderr, "%f ", H_l(a,b));
        //            }
        //            fprintf(stderr, "\n");
        //        }
        fprintf(stderr, "H2 numerical\n");
        for(int a = 0; a < 2; a++) {
            for(int b = 0; b < 3; b++) {
                fprintf(stderr, "%f ", num_H2(a,b));
            }
            fprintf(stderr, "\n");
        }
        fprintf(stderr, "\n");
    ...
    template <class Derived0, class Derived1, class Derived2>
    static void quaternionJacobian(const Eigen::MatrixBase<Derived0> & q,
                                   const Eigen::MatrixBase<Derived1> & t,
                                   Eigen::MatrixBase<Derived2> & Jacob) {

        DimensionCheck<Eigen::Vector4d>(q);
        DimensionCheck<Eigen::Vector3d>(t);
        DimensionCheck<Eigen::Matrix<double, 3, 4> >(Jacob);

        //        Eigen::Vector4d v1;
        //        v1 << 1.0, -1.0, -1.0, -1.0;
        //        Eigen::Matrix4d I_star = v1.asDiagonal();

        //        Eigen::Vector4d q_conj;
        //        q_conj << q[0], -q[1], -q[2], -q[3];

        //        Jacob = C3DJacobians::t_Quat_to_SkewMatrix( C3DJacobians::t_Quat_Production(q, t) ) * I_star +
        //                C3DJacobians::t_Quat_to_SkewHatMatrix( C3DJacobians::t_Quat_Production(t, q_conj) );
        ...
        //        Eigen::Matrix<double, 3, 3> dR_by_dq0, dR_by_dqx, dR_by_dqy, dR_by_dqz;
        ...
        //        Jacob.Zero();
        //        Jacob.block(0,0,3,1) = dR_by_dq0 * t;
        //        Jacob.block(0,1,3,1) = dR_by_dqx * t;
        //        Jacob.block(0,2,3,1) = dR_by_dqy * t;
        //        Jacob.block(0,3,3,1) = dR_by_dqz * t;

        //        fprintf(stderr, "Jacob v1\n");
        //        for (size_t i=0; i<3; ++i) {
        //            for (size_t j=0; j<4; ++j) {
        //                fprintf(stderr, "%f ", Jacob(i,j));
        //            }
        //            fprintf(stderr, "\n");
        //        }

        Eigen::Matrix<double, 4, 3> Pi;
        Pi << -qx, -qy, -qz,
                q0, -qz,  qy,
                qz,  q0, -qx,
                -qy,  qx,  q0;

        Eigen::Vector4d sc = 2*Pi*t;

        Jacob << sc(1), sc(0), -sc(3), sc(2),
                sc(2), sc(3), sc(0), -sc(1),
                sc(3), -sc(2), sc(1), sc(0);

        //        fprintf(stderr, "Jacob v2\n");
        //        for (size_t i=0; i<3; ++i) {
        //            for (size_t j=0; j<4; ++j) {
        //                fprintf(stderr, "%f ", Jacob(i,j));
        //            }
    ...
    // TODO
    // figure out approximation of se3 jacobian
    template <class Derived0, class Derived1, class Derived2, class Derived3>
    static void lieAlgebraJacobian(const Eigen::MatrixBase<Derived0> & RaT,
                                   const Eigen::MatrixBase<Derived1> & thi,
                                   const Eigen::MatrixBase<Derived2> & t,
                                   Eigen::MatrixBase<Derived3> & Jacob) {
        //
        DimensionCheck<Eigen::Matrix<double, 3, 4> >(RaT);
        DimensionCheck<Eigen::Vector3d>(thi);
        DimensionCheck<Eigen::Vector3d>(t);
        DimensionCheck<Eigen::Matrix<double, 3, 3> >(Jacob);

        double theta = thi.norm();
        ...
        if (fabs(theta) > 1e-5) {
            ...
            I33.setIdentity();
            Eigen::Matrix<double, 3, 3> thi_skew;
            thi_skew << 0, -thi(2), thi(1),
                    thi(2), 0, -thi(0),
                    -thi(1), thi(0), 0;
            Eigen::Matrix<double, 3, 3> Jr = I33 - (1 - cos(theta)) / theta_2 * thi_skew +
                    (theta - sin(theta)) / theta_3 * thi_skew * thi_skew;

            Eigen::Matrix<double, 3, 3> t_skew;
            t_skew << 0, -t(2), t(1),	// this constructs skew symmetric matrix [t]_x
                    t(2), 0, -t(0),
                    -t(1), t(0), 0;
            Jacob = - RaT.template block<3, 3>(0, 0) * t_skew * Jr;
        }
        else {
            Eigen::Matrix<double, 3, 3> t_skew;
            t_skew << 0, -t(2), t(1),	// this constructs skew symmetric matrix [t]_x
     ...
     *	@note Neither of the input parameters may be reused as output.
     */
#ifdef ENABLE_ANALYTICAL_JACOBIAN
    template <class Derived0, class Derived1, class Derived2, class Derived3, class Derived4, class Derived5> // want to be able to call this with differnent dest types (sometimes generic VectorXd / MatrixXd, sometimes with Vector3d / Matrix3d)
    static void Project_P2C(const Eigen::MatrixBase<Derived0> &r_t_vertex1,
                            const Eigen::MatrixBase<Derived1> &r_t_intrinsics,
                            const Eigen::MatrixBase<Derived2> &r_t_vertex2, Eigen::MatrixBase<Derived3> &r_t_dest,
                            Eigen::MatrixBase<Derived4> &r_t_pose3_pose1, Eigen::MatrixBase<Derived5> &r_t_pose3_pose2)
    {
        ...
        //construct [R | t]
        Eigen::Matrix<double, 3, 4> RaT;
        RaT.template block<3, 3>(0, 0) = C3DJacobians::t_AxisAngle_to_RotMatrix(r_t_vertex1.template tail<3>());
        RaT.col(3) = r_t_vertex1.template head<3>();
        ...
        Eigen::Matrix<double, 3, 3> dRq_times_a_by_dq;
        lieAlgebraJacobian(RaT, r_t_vertex1.template tail<3>(), r_t_vertex2-r_t_vertex1.template head<3>(),
                           dRq_times_a_by_dq);

        //        Eigen::Vector3d tmp_vec = - R_t * (r_t_vertex2-r_t_vertex1.template head<3>());
        //        dRq_times_a_by_dq(0, 0) = 0.0f;
        //        dRq_times_a_by_dq(1, 0) = tmp_vec(2);
        //        dRq_times_a_by_dq(2, 0) = -tmp_vec(1);
        //        //
        //        dRq_times_a_by_dq(0, 1) = -tmp_vec(2);
        //        dRq_times_a_by_dq(1, 1) = 0.0f;
        //        dRq_times_a_by_dq(2, 1) = tmp_vec(0);
        //        //
        //        dRq_times_a_by_dq(0, 2) = tmp_vec(1);
        //        dRq_times_a_by_dq(1, 2) = -tmp_vec(0);
        //        dRq_times_a_by_dq(2, 2) = 0.0f;

        r_t_pose3_pose1.template block<2, 3>(0, 3) = U_p * dRq_times_a_by_dq;
        ...
        Eigen::Vector2d d1;
        Vector6d p_delta;
        Eigen::Matrix<double, 2, 6> num_H1;

        //for so3, use numertical estimation
        for(int j = 3; j < 6; ++ j) {
