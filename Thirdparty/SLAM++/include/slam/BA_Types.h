...
/**
 *	@brief bundle adjustment camera pose with quaternion
 *         compared with se3/so3 parameterization, quaternion
 *         is much easier to derive Jacobian analatically
 */
class CVertexCam_Quat : public CBaseVertexImpl<CVertexCam_Quat, 7> {
protected:
...
public:
     ...
     */
    inline CVertexCam_Quat(const Eigen::Matrix<double, 12, 1> &r_v_state)
        :CBaseVertexImpl<CVertexCam_Quat, 7>(r_v_state.head<7>()),
     ...
     *	@param[in] r_v_state is state of the vertex
     */
    inline CVertexCam_Quat(const Eigen::Matrix<double, 7, 1> &r_v_state)
        :CBaseVertexImpl<CVertexCam_Quat, 7>(r_v_state.head<7>())
    {
     ...
     */
    inline CVertexCam_Quat(const CParserBase::TVertexCam3D &r_t_vertex)
        :CBaseVertexImpl<CVertexCam_Quat, 7>(r_t_vertex.m_v_position.head<7>()),
    ...
    {
        // TODO
        // impl Relative_to_Absolute for quaternions
        //        C3DJacobians::Relative_to_Absolute(m_v_state, r_v_delta.segment<7>(m_n_order), m_v_state);
    ...
    inline void Operator_Minus(const Eigen::VectorXd &r_v_delta) // "smart" minus
    {
        ...
        //        C3DJacobians::Relative_to_Absolute(m_v_state, -r_v_delta.segment<7>(m_n_order), m_v_state); // avoid calculating negative of the whole r_v_delta
    }
};
 ...
 *         note that though we can set intrisic parameters per camera vertex,
 *         they won't be optimized in BA
 */
...
/**
 *	@brief BA vex-cam edge, with simplified Jacobian computation
 *         for good graph only; not recommended in actual optimization
 */
class CEdgeP2C3D_Quat : public CBaseEdgeImpl<CEdgeP2C3D_Quat, MakeTypelist(CVertexCam_Quat, CVertexXYZ), 2> {
    public:
    ...
    CEdgeP2C3D_Quat(const CParserBase::TEdgeP2C3D &r_t_edge, CSystem &r_system)
        :CBaseEdgeImpl<CEdgeP2C3D_Quat, MakeTypelist(CVertexCam_Quat, CVertexXYZ), 2>(r_t_edge.m_n_node_1,
     ...
     */
    template <class CSystem>
    CEdgeP2C3D_Quat(size_t n_node1, size_t n_node0, const Eigen::Vector2d &v_delta, // won't align non-reference arg
                    const Eigen::Matrix2d &r_t_inv_sigma, CSystem &r_system) // respect const-ness!
        :CBaseEdgeImpl<CEdgeP2C3D_Quat, MakeTypelist(CVertexCam_Quat, CVertexXYZ), 2>(n_node0,
    ...
    {
        CBaseEdgeImpl<CEdgeP2C3D_Quat, MakeTypelist(CVertexCam_Quat, CVertexXYZ),
     ...
     */
    inline void Calculate_Jacobians_Expectation_Error(Eigen::Matrix<double, 2, 7> &r_t_jacobian0,
    ...
    {
        // NOTE
        // here is the only difference between CEdgeP2C3D_Quat and CEdgeP2C3D;
        // instead of deriving Jacobian with se3, analatical formulation is utilzied
        // with quaternion formulation
        CBAJacobians::Project_P2C_Quat(m_p_vertex0->r_v_State() /* Cam */, m_p_vertex0->v_Intrinsics() /* camera intrinsic params */,
    ...
    inline double f_Chi_Squared_Error() const
    {
        ...
        CBAJacobians::Project_P2C_Quat(m_p_vertex0->r_v_State() /* Cam */, m_p_vertex0->v_Intrinsics(),
