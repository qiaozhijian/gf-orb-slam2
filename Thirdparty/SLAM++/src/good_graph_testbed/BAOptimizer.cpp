...
#include "slam/NonlinearSolver_GoodGraph.h"
//#include "slam/BASolverBase.h" // included from BA_Types.h
#include "good_graph_testbed/BAOptimizer.h"
...
public:
    ...
    typedef MakeTypelist_Safe((CVertexSCam, CVertexXYZ)) TVertexTypelist;
    typedef MakeTypelist_Safe((CEdgeP2SC3D)) TEdgeTypelist;
    typedef MakeTypelist_Safe((CVertexSCam, CVertexXYZ)) TConstVertexTypelist; // a new list of constant vertices
    typedef CFlatSystem<CBaseVertex, TVertexTypelist, CEdgeP2SC3D, TEdgeTypelist, CBasicUnaryFactorFactory, CBaseVertex, TConstVertexTypelist> CSystemType;
#elif defined ENABLE_QUATERNION
    typedef MakeTypelist_Safe((CVertexCam_Quat, CVertexXYZ)) TVertexTypelist;
    typedef MakeTypelist_Safe((CEdgeP2C3D_Quat)) TEdgeTypelist;
    typedef MakeTypelist_Safe((CVertexCam_Quat, CVertexXYZ)) TConstVertexTypelist; // a new list of constant vertices
    typedef CFlatSystem<CBaseVertex, TVertexTypelist, CEdgeP2C3D_Quat, TEdgeTypelist, CBasicUnaryFactorFactory, CBaseVertex, TConstVertexTypelist> CSystemType;
#else
    typedef MakeTypelist_Safe((CVertexCam, CVertexXYZ)) TVertexTypelist;
    typedef MakeTypelist_Safe((CEdgeP2C3D)) TEdgeTypelist;
    typedef MakeTypelist_Safe((CVertexCam, CVertexXYZ)) TConstVertexTypelist; // a new list of constant vertices
    typedef CFlatSystem<CBaseVertex, TVertexTypelist, CEdgeP2C3D, TEdgeTypelist, CBasicUnaryFactorFactory, CBaseVertex, TConstVertexTypelist> CSystemType;
#endif
    ...
    typedef CNonlinearSolver_GoodGraph<CSystemType, CLinearSolverType> CNonlinearSolverType;
...
//    return m_p_optimizer->r_Solver().Plot3D(p_s_filename, plot_quality::plot_Printing);
    return m_p_optimizer->r_Solver().Plot3D(p_s_filename);
}

void CBAOptimizer::resetTime() {
    return m_p_optimizer->r_Solver().resetTime();
}
...
{
    m_p_optimizer->r_System().r_Get_ConstVertex<CVertexXYZ>(n_vertex_id, v_position);
}
...
void CBAOptimizer::Add_SCamVertex_Fixed(size_t n_vertex_id, const Eigen::Matrix<double, 12, 1> &v_cam_state) // throw(srd::bad_alloc)
{
    m_p_optimizer->r_System().r_Get_ConstVertex<CVertexSCam>(n_vertex_id, v_cam_state);
}
...
void CBAOptimizer::Add_CamVertex(size_t n_vertex_id, const Eigen::Matrix<double, 12, 1> &v_cam_state) // throw(srd::bad_alloc)
{
    m_p_optimizer->r_System().r_Get_Vertex<CVertexCam_Quat>(n_vertex_id, v_cam_state);
}

void CBAOptimizer::Add_CamVertex_Fixed(size_t n_vertex_id, const Eigen::Matrix<double, 12, 1> &v_cam_state) // throw(srd::bad_alloc)
{
    m_p_optimizer->r_System().r_Get_ConstVertex<CVertexCam_Quat>(n_vertex_id, v_cam_state);
}
...
{
    m_p_optimizer->r_System().r_Add_Edge(CEdgeP2C3D_Quat(n_xyz_vertex_id, n_cam_vertex_id,
                                                         v_observation, t_information, m_p_optimizer->r_System()));
}
...
void CBAOptimizer::Add_CamVertex_Fixed(size_t n_vertex_id, const Eigen::Matrix<double, 11, 1> &v_cam_state) // throw(srd::bad_alloc)
{
    m_p_optimizer->r_System().r_Get_ConstVertex<CVertexCam>(n_vertex_id, v_cam_state);
}
...
void CBAOptimizer::Dump_TimeLog(double & m_f_lambda_time, double & m_f_pre_time, double & m_f_schur_time, double & m_f_query_time,
    ...
    //
    m_f_lambda_time = m_p_optimizer->r_Solver().m_f_lambda_time;
    m_f_pre_time    = m_p_optimizer->r_Solver().m_f_pre_time;
    m_f_schur_time  = m_p_optimizer->r_Solver().m_f_schur_time;
    m_f_query_time  = m_p_optimizer->r_Solver().m_f_query_time;
    m_f_slice_time  = m_p_optimizer->r_Solver().m_f_slice_time;
    m_f_chol_time   = m_p_optimizer->r_Solver().m_f_chol_time;
    m_f_post_time   = m_p_optimizer->r_Solver().m_f_post_time;
}
...
public:
    ...
    void operator ()(const CVertexSCam &r_vertex_cam)
    {
        const Eigen::Matrix<double, 6, 1> &r_v_state = r_vertex_cam.r_v_State();
        const Eigen::Matrix<double, 6, 1> &r_v_intrinsics = r_vertex_cam.v_Intrinsics();
        ...
        fprintf(m_p_fw, "VERTEX_SCAM " PRIsize " %.15f %.15f %.15f %.15g %.15g %.15g %.15g %.15f %.15f %.15f %.15f %.15f %.15f\n", // use %g for the quat, as it will have less than zero numbers
                ...
                r_v_intrinsics(3), r_v_intrinsics(4), r_v_intrinsics(5));
    }

    void operator ()(const CEdgeP2SC3D &r_t_edge)
    {
        size_t n_point_id = r_t_edge.n_Vertex_Id(1);
        size_t n_camera_id =  r_t_edge.n_Vertex_Id(0); // someone swapped this in the edge type

        const CEdgeP2SC3D::_TyVectorAlign &r_v_measurement = r_t_edge.v_Measurement();
        const CEdgeP2SC3D::_TyMatrixAlign &r_t_information = r_t_edge.t_Sigma_Inv();

        fprintf(m_p_fw, "EDGE_PROJECT_P2SMC " PRIsize " " PRIsize " %f %f %f %g %g %g %g %g %g\n", // these do not need to be very precise (x, y detected by SIFT with barely any fractional precision, covariance typically large integer numbers)
                n_point_id, n_camera_id, r_v_measurement(0), r_v_measurement(1), r_v_measurement(2),
                r_t_information(0, 0), r_t_information(0, 1), r_t_information(0, 2),
                r_t_information(1, 1), r_t_information(1, 2),
                r_t_information(2, 2)); // only the upper triangular part is stored
    }

#elif defined ENABLE_QUATERNION

    void operator ()(const CVertexCam_Quat &r_vertex_cam)
    {
...
//        Eigen::Quaterniond t_rotation;
//        C3DJacobians::AxisAngle_to_Quat(r_v_state.tail<3>(), t_rotation);
        Eigen::Quaterniond t_rotation(r_v_state.tail<4>());
        t_rotation = t_rotation.conjugate(); // R^-1
        Eigen::Vector3d v_translation = -t_rotation._transformVector(r_v_state.head<3>()); // -R^-1 * t
        // the convention in the graph file is inverse opposite to the internal state
    ...
    void operator ()(const CEdgeP2C3D_Quat &r_t_edge)
    {
        size_t n_point_id = r_t_edge.n_Vertex_Id(1);
        size_t n_camera_id =  r_t_edge.n_Vertex_Id(0); // someone swapped this in the edge type

        const CEdgeP2C3D_Quat::_TyVectorAlign &r_v_measurement = r_t_edge.v_Measurement();
        const CEdgeP2C3D_Quat::_TyMatrixAlign &r_t_information = r_t_edge.t_Sigma_Inv();
...
void Add_XYZVertex_Fixed(optimizer_t h_optimizer, size_t n_vertex_id, const double p_position[3])
{
    CBAOptimizer &r_opt = *reinterpret_cast<CBAOptimizer*>(h_optimizer);
    r_opt.Add_XYZVertex_Fixed(n_vertex_id, Eigen::Map<const Eigen::Vector3d, Eigen::DontAlign>(p_position));
}

#ifdef ENABLE_STEREO_SLAMPP

void Add_SCamVertex(optimizer_t h_optimizer, size_t n_vertex_id, const double p_cam_state[12])
{
    CBAOptimizer &r_opt = *reinterpret_cast<CBAOptimizer*>(h_optimizer);
    r_opt.Add_SCamVertex(n_vertex_id, Eigen::Map<const Eigen::Matrix<double, 12, 1>, Eigen::DontAlign>(p_cam_state));
}

void Add_SCamVertex_Fixed(optimizer_t h_optimizer, size_t n_vertex_id, const double p_cam_state[12])
{
    CBAOptimizer &r_opt = *reinterpret_cast<CBAOptimizer*>(h_optimizer);
    r_opt.Add_SCamVertex_Fixed(n_vertex_id, Eigen::Map<const Eigen::Matrix<double, 12, 1>, Eigen::DontAlign>(p_cam_state));
}
...
{
    CBAOptimizer &r_opt = *reinterpret_cast<CBAOptimizer*>(h_optimizer);
    r_opt.Add_P2SC3DEdge(n_xyz_vertex_id, n_cam_vertex_id,
                         Eigen::Map<const Eigen::Vector3d, Eigen::DontAlign>(p_observation),
                         Eigen::Map<const Eigen::Matrix3d, Eigen::DontAlign>(p_information));
}

#elif defined ENABLE_QUATERNION

void Add_CamVertex(optimizer_t h_optimizer, size_t n_vertex_id, const double p_cam_state[12])
{
    CBAOptimizer &r_opt = *reinterpret_cast<CBAOptimizer*>(h_optimizer);
    r_opt.Add_CamVertex(n_vertex_id, Eigen::Map<const Eigen::Matrix<double, 12, 1>, Eigen::DontAlign>(p_cam_state));
}

void Add_CamVertex_Fixed(optimizer_t h_optimizer, size_t n_vertex_id, const double p_cam_state[12])
{
    CBAOptimizer &r_opt = *reinterpret_cast<CBAOptimizer*>(h_optimizer);
    r_opt.Add_CamVertex_Fixed(n_vertex_id, Eigen::Map<const Eigen::Matrix<double, 12, 1>, Eigen::DontAlign>(p_cam_state));
}
...
void Add_CamVertex_Fixed(optimizer_t h_optimizer, size_t n_vertex_id, const double p_cam_state[11])
{
    CBAOptimizer &r_opt = *reinterpret_cast<CBAOptimizer*>(h_optimizer);
    r_opt.Add_CamVertex_Fixed(n_vertex_id, Eigen::Map<const Eigen::Matrix<double, 11, 1>, Eigen::DontAlign>(p_cam_state));
}
